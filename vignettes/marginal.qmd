...
p4 <- gggenomes(seqs = emale_seqs, genes = emale_genes, links = emale_ava) +
  geom_seq() +
  geom_link(aes(fill=if_else(.marginal, "marginal", NA))) +
  geom_gene(aes(fill=case_when(
    str_detect(feat_id, "10$") ~ "target",
    .marginal ~ "marginal",
    .default = NA
  ))) +
  geom_seq_label() +
...
p5 <- p4 |> 
  focus(str_detect(feat_id, "10$"), .expand = 1200) +
...
p8 <- p4 |> focus(str_detect(feat_id, "10$"), .expand = 1200,
