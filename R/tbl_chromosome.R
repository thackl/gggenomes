#' @export
as_tbl_chromosome <- function(data, ...){
    UseMethod("as_tbl_chromosome")
}

#' @export
as_tbl_chromosome.default <- function(data, ...) {
    # try to coerce into tbl
    as_tbl_chromosome(as_tibble(data), ...)
}

#' @export
as_tbl_chromosome.tbl_df <- function(
    ...
    class(data) <- c("tbl_chromosome", class(data))
    data
}
