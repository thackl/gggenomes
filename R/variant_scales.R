...
#' (These default make most sense when using `geom_variant(offset = -0.2)`).      
...
#' The functions `scale_color_variant()` and `scale_shape_variant()` have a default setting, which can be overwrite.
...
scale_shape_variant <- function(values=c(SNP=23, Deletion=25, Insertion=24), characters = FALSE, ...){
