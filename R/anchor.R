...
#' @param .align alignment position, one of "left", "center", "right" or numeric
...
#' 
#' pp + plot_layout(guides = "collect") & geom_vline(xintercept = 0, linetype = 2)
align <- function(x, ..., .track_id = "genes", .align = "left"){
  ...
  a <- x |> 
    ...
    dplyr::summarize(x = ((1-.align) * min(c(x, xend))) + (.align * max(c(x, xend))))
